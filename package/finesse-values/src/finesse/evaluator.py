...
class FinesseEvaluator:
    ...
    def merger_run(self) -> Dict[str, Any]:
        ...
        length_results = {}
        target_lengths = self.config.probe_config.sequence_length.get_lengths()
        for target_length in tqdm(target_lengths, desc="Merger RSS: Probing Lengths"):
    ...
    def merger_run_srs(self) -> Dict[str, Any]:
        ...
        for target_length in target_lengths:
            ...
            while len(sample_results) < self.config.probe_config.samples_per_length:
                attempts += 1
                if attempts > max_attempts:
                    raise ValueError(f"dataset exhaustion: target_length={target_length} not enought sample. {len(sample_results)}/{self.config.probe_config.samples_per_length}")
                typer.echo(f"probe sequence [{target_length}] in progress ({len(sample_results)}/{self.config.probe_config.samples_per_length})...")                
    ...
    def native_run_srs(self) -> Dict[str, Any]:
        ...
        length_results = {}
        target_lengths = self.config.probe_config.sequence_length.get_lengths()
        for target_length in tqdm(target_lengths, desc="Native SRS: Probing Lengths"):
    ...
    def _dataset_prepare_and_validate(self, mode: str = 'rss') -> dict:
        ...
        lengths = probe_config.sequence_length.get_lengths()
        if mode == 'rss':
            total_chunks_required = sum(L * probe_config.samples_per_length for L in lengths)
        elif mode == 'srs':
            total_chunks_required = sum(
                (
                    (L - 1) + (probe_config.group_amount * (L - 2))
                ) * probe_config.samples_per_length
                for L in lengths if L >= 2
            )
        else:
